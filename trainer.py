...
class BaseTrainer:
    ...
    def train_step(self): #la til model her
        """
    ...
    def train( 
            self,
            num_epochs: int): #la til model her
        """
        ...
        global_step = 0
        # best loss starts at maximum possible value (can only go downwards)
        bestLoss = float("inf")
        numEpochsDone = 0
        bestLossArr = []
        # best accuracy starts at 0, can only go upwards
        bestAcc = 0
        for epoch in range(num_epochs):
            ...
            for X_batch, Y_batch in iter(train_loader):
                ...
                if global_step % num_steps_per_val == 0:
                    ...
                    #early stopping:
                    bestThisEpoch = np.min(val_history["loss"][global_step]) #the best validation loss this epoch
                    if (bestLoss > bestThisEpoch):
                        bestLoss = bestThisEpoch
                    bestLossArr.append(bestThisEpoch)
                    if (len(bestLossArr) > 50): #we do not start to check before we actually have 10 values of val_loss
                        if (bestLoss not in bestLossArr[-50:]):
                            print("epochs when early stop: ", numEpochsDone)
                            return train_history, val_history
                global_step += 1
        print("Epochs done: ", numEpochsDone)
        return train_history, val_history
